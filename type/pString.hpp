...
template <typename T>
class String : public Reflect<String<T>>, public pTypeObject<T, String<T>> {
  shared_ptr<T> buffer;
  int length;
  int size;
...
public:
  ...
  static const String<T> LToS(long long value) {
    ...
    int index = length - 1;
    while (index >= 0) {
      tempbuffer[length - (negative ? 0 : 1) - index] =
          '0' + value / pow(10, index) % 10;
      index--;
    }
    if (negative)
      tempbuffer[0] = '-';
  ...
  String(long value) { *this = LToS(value); }

  String(int value) { *this = LToS(value); }
  ...
  String(const String<T> &src, int start, int end) {
    CONDITION_MESSAGE(start > src.length || start < 0 || end < 0 ||
                          end > src.length || end < start,
  ...
  //     CONDITION_MESSAGE(ilength>src.length || ilength<0,"invalid length
  //     param!"); length = ilength; buffer = shared_ptr<T>(new
  //     T[length+1],default_delete<T[]>()); size = sizeof(T)*length;
  ...
  unsigned int _size() const { return size; }
  unsigned int _length() const { return length; }
  ...
  const T *c_str() const { return buffer.get(); }
  ...
  int find(const String<T> &src) const {
    ...
    while (pos < length) {
      if ((buffer.get())[pos] == (src.buffer.get())[0]) {
        if (pos + src.length > length)
          break;
        for (int index = 0; index < src.length; index++)
          if ((buffer.get())[pos + index] != (src.buffer.get())[index])
            goto goon;
        return pos;
      }
    goon:
  ...
  bool Contain(const String<T> &src) const { return !(-1 == this->find(src)); }

  bool StartWith(const String<T> &src) const { return find(src) == 0; }

  bool EndWith(const String<T> &src) const {
    return rfind(src) + src.length == length;
  }

  bool Empty() { return Empty(*this); }
  ...
  long ToLong() const { return ToL(*this); }

  const String<T> &replace(const String<T> &src, const String<T> &sbj) {
    auto pos = -1;
    while (-1 != (pos = find(src))) {
      *this = substr(0, pos) + sbj +
              substr(pos + src._length(),
                     _length() - substr(0, pos)._length() - src._length());
    }
    return *this;
  }

  String<T> substr(int pos) { return String<T>(*this, pos, length - 1); }
  ...
  KeyValuePair<> Cut(const String<T> &split, long cutpos = -1) const {
    ...
    while (count > 0) {
      ...
      if (pos == -1)
        break;
      temp = cutpos < 0 ? String<T>(temp, 0, pos)
                        : String<T>(temp, pos + 1, temp.length - 1);
  ...
  CL Split(CL slist) const {
    ...
    for (; index < length; index++) {
      for (int indeY = 0; indeY < slist.size; indeY++) {
        ...
        for (int indeZ = 0; indeZ < slist[indeY].size && indeY + indeZ < length;
             indeZ++) {
  ...
  bool operator==(const String<T> &des) const {
    ...
    if (size != des.size && length != des.length)
      return false;
    return Compare(*this, des) == 0;
  }
  bool operator!=(const String<T> &dec) const { return !(dec == *this); }
  const String<T> &operator+=(const String<T> &src) {
    *this = *this + src;
    return *this;
  }
...
template <typename T> bool is_same_class(const AString &classname) {
