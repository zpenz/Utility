...
next:
    ...
    static void Test(const AString& GenPath=""){
        ...
        for(int index=0;index<TableSize;index++){
            ...
            //wrapper
            AString WrpperCode = "<?php \n    include_once(\'"+FileName+"');\n\n    class "+ClassName+" extends _"+ClassName+"{\n\n    };\n";
            auto WrpperFile = fopen(WrpperFileName,"r");
            if(WrpperFile) {
                SAFE_CLOSE(WrpperFile);
                continue;
            }
            WrpperFile = fopen(WrpperFileName,"wr");
            fwrite(WrpperCode.c_str(),sizeof(char),WrpperCode._length(),WrpperFile);
            SAFE_CLOSE(WrpperFile);
        }
    }
};
